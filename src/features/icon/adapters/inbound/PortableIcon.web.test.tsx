...
await mock.module('react-native', () => ({ ...ReactNativeWeb, Image: WebImageWithoutResolveAssetSource }));
await mock.module('react-native-svg', () => ({
  SvgUri: ({ color, height, uri, width }: Record<string, unknown>) =>
    React.createElement('svg', { 'data-color': color, 'data-uri': uri, height, width }),
}));
...
describe('portable icon rendering', () => {
  test('renders Ionicons through the public Icon API', () => {
    expect(renderToStaticMarkup(<Icon color="#123456" name="home-outline" size={18} />)).toContain(
      'font-family:Ionicons',
    );
  });
  test('renders FontAwesome through the public Icon API', () => {
    expect(
      renderToStaticMarkup(<Icon color="#123456" name="github" provider="FontAwesome" size={18} />),
    ).toContain('font-family:FontAwesome');
  });
  test('renders a FontAwesome5 brand icon through its brand font', () => {
    ...
    ).toContain('font-family:FontAwesome5Brands-Regular');
  });
  test('renders a FontAwesome5 solid icon through its solid font', () => {
    ...
    ).toContain('font-family:FontAwesome5Free-Solid');
  });
  test('renders a FontAwesome6 brand icon through its brand font', () => {
    ...
    ).toContain('font-family:FontAwesome6Brands-Regular');
  });
  test('renders Material Design Icons through the public Icon API', () => {
    ...
    ).toContain('font-family:MaterialDesignIcons');
  });
...
describe('portable icon runtime rejection', () => {
  test('throws for an invalid runtime provider instead of falling back to Ionicons', () => {
    const props = { color: '#123456', name: 'home-outline', size: 18 } satisfies Parameters<
      typeof PortableIcon
    >[0];
